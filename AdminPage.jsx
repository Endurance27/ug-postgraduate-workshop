...
import {
  ...
  Film, HelpCircle, Eye, EyeOff, ArrowRight, ArrowLeft,
  Check, X, Upload, Download, Plus, Trash2, AlertCircle, User,
  ClipboardList, CreditCard, TrendingUp, AlertTriangle, CheckCircle2,
...
function PaymentTrackingPanel({ payments = [], fee = 100, onChange }) {
  ...
  const [addForm, setAddForm]     = useState({
    name: "", email: "", programme: "", studentId: "", transactionId: "",
    amount: fee, method: "mobile_money", date: "", status: "Confirmed",
  });
  ...
  const confirmedAmt  = confirmed.reduce((s, p) => s + (Number(p.amount) || 0), 0);
  ...
  const visible = payments
    ...
    .filter(p => {
      ...
      return (p.transactionId || "").toLowerCase().includes(q)
  ...
  const exportCSV = () => {
    const header = "Transaction ID,Student ID,Name,Email,Programme,Amount (GHS),Method,Date,Status";
    const rows = payments.map(p =>
      [p.transactionId, p.studentId, p.name, p.email, p.programme, p.amount,
       p.method, fmtDate(p.date), p.status].map(v => `"${v || ""}"`).join(",")
    );
    const blob = new Blob([[header, ...rows].join("\n")], { type: "text/csv" });
    const url  = URL.createObjectURL(blob);
    const a    = Object.assign(document.createElement("a"), { href: url, download: "payments.csv" });
  ...
  const addPayment = () => {
    ...
    setAddForm({ name: "", email: "", programme: "", studentId: "", transactionId: "", amount: fee, method: "mobile_money", date: "", status: "Confirmed" });
    setShowAdd(false);
  };
  ...
  const SortIcon = ({ k }) => {
    if (sortKey !== k) return <span style={{ color: "#ccc", fontSize: 11 }}>↕</span>;
    return sortDir === "asc"
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
        <div>
          <h2 style={{ marginBottom: 4, fontFamily: "Playfair Display, serif" }}>Payment Tracking</h2>
          <p style={{ color: "#666", fontSize: 14 }}>Monitor and manage all transaction records</p>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={exportCSV} style={{
            display: "flex", alignItems: "center", gap: 6,
            background: "#fff", color: "#1B3A6B", border: "1.5px solid #b0bdd8",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px,1fr))", gap: 16, marginBottom: 28 }}>
        {cards.map((c, i) => (
          <div key={i} style={{ background: "#fff", border: "1px solid #e8eaf0", borderRadius: 14, padding: "20px 22px", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              <div style={{ fontSize: 13, color: "#888", marginBottom: 8 }}>{c.label}</div>
              <div style={{ fontSize: 26, fontWeight: 800, color: "#0F2347", fontFamily: "Playfair Display, serif" }}>{c.value}</div>
              {c.sub && <div style={{ fontSize: 13, fontWeight: 600, color: c.subColor, marginTop: 4 }}>{c.sub}</div>}
            </div>
      ...
      {/* Add payment form */}
      {showAdd && (
        <div className="card" style={{ marginBottom: 20, border: "2px solid #C9A84C" }}>
          <h4 style={{ marginBottom: 16, fontFamily: "Playfair Display, serif" }}>Add Manual Payment Record</h4>
          <div className="form-row">
            <div className="form-group"><label>Full Name *</label><input value={addForm.name} onChange={e => setAddForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Kwame Asante" /></div>
            <div className="form-group"><label>Email</label><input type="email" value={addForm.email} onChange={e => setAddForm(f => ({ ...f, email: e.target.value }))} placeholder="e.g. kwame@ug.edu.gh" /></div>
          </div>
          <div className="form-row">
            <div className="form-group"><label>Transaction ID *</label><input value={addForm.transactionId} onChange={e => setAddForm(f => ({ ...f, transactionId: e.target.value }))} placeholder="e.g. UGPGW2026-PAY-..." /></div>
            <div className="form-group"><label>Student ID</label><input value={addForm.studentId} onChange={e => setAddForm(f => ({ ...f, studentId: e.target.value }))} placeholder="e.g. 10823456" /></div>
          </div>
          <div className="form-row">
            <div className="form-group"><label>Amount (GHS)</label><input type="number" value={addForm.amount} onChange={e => setAddForm(f => ({ ...f, amount: Number(e.target.value) }))} /></div>
            <div className="form-group">
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
            <button className="btn-primary" onClick={addPayment}><span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><Check size={14} /> Save Record</span></button>
            <button onClick={() => setShowAdd(false)} style={{ background: "#f0f0f0", border: "none", borderRadius: 8, padding: "9px 16px", fontSize: 13, cursor: "pointer" }}>Cancel</button>
          </div>
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap", alignItems: "center" }}>
        <div style={{ position: "relative", flex: 1, minWidth: 240 }}>
          ...
          <input
            ...
            style={{ width: "100%", paddingLeft: 36, paddingRight: 12, paddingTop: 9, paddingBottom: 9, border: "1.5px solid #ddd", borderRadius: 10, fontSize: 13 }}
          />
        </div>
        <div style={{ display: "flex", gap: 6 }}>
          {["All", "Confirmed", "Pending"].map(f => (
            <button key={f} onClick={() => setFilter(f)} style={{
              ...
              border: "1.5px solid #ddd", borderRadius: 20, padding: "6px 16px", fontSize: 13, cursor: "pointer", fontWeight: filter === f ? 600 : 400,
            }}>{f}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ background: "#fff", border: "1px solid #e0e0e0", borderRadius: 14, overflow: "hidden" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ background: "#f8f9fa", borderBottom: "1px solid #eee" }}>
                {[
                  ...
                  <th key={i} onClick={col.key ? () => toggle(col.key) : undefined}
                    style={{ padding: "13px 16px", textAlign: "left", fontWeight: 600, fontSize: 12, color: "#555", whiteSpace: "nowrap", cursor: col.key ? "pointer" : "default", userSelect: "none" }}>
            ...
            <tbody>
              {visible.length === 0 && (
                <tr><td colSpan={8} style={{ textAlign: "center", padding: "40px", color: "#888" }}>No payment records found.</td></tr>
              )}
              {visible.map((p, i) => (
                <tr key={p.id} style={{ borderBottom: "1px solid #f5f5f5", background: i % 2 === 0 ? "#fff" : "#fafafa" }}>
                  <td style={{ padding: "13px 16px", maxWidth: 200 }}>
                    <span style={{ color: "#1B3A6B", fontFamily: "monospace", fontSize: 11, cursor: "pointer", textDecoration: "underline" }}
                      onClick={() => setSelected(p)}>
                      {(p.transactionId || "—").slice(0, 28)}{(p.transactionId || "").length > 28 ? "…" : ""}
                    </span>
                  </td>
                  <td style={{ padding: "13px 16px", color: "#555", fontWeight: 500 }}>{p.studentId || "—"}</td>
                  <td style={{ padding: "13px 16px" }}>
                    <div style={{ fontWeight: 500 }}>{p.name}</div>
                    <div style={{ fontSize: 11, color: "#888" }}>{p.email}</div>
                  </td>
                  <td style={{ padding: "13px 16px", color: "#555", whiteSpace: "nowrap" }}>
                    <div>{p.date ? new Date(p.date).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }) : "—"}</div>
                  ...
                  <td style={{ padding: "13px 16px" }}>
                    <button onClick={() => setSelected(p)} style={{
                      ...
                      background: "#f4f6fb", color: "#1B3A6B", border: "1px solid #b0bdd8",
                      borderRadius: 8, padding: "5px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer",
                    }}><Eye size={13} /> View Details</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div style={{ padding: "12px 20px", borderTop: "1px solid #eee", fontSize: 12, color: "#888" }}>
        ...
        <div style={{
          ...
          <div style={{ background: "#fff", borderRadius: 16, padding: 32, maxWidth: 520, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.25)" }}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
              <h3 style={{ fontFamily: "Playfair Display, serif", margin: 0 }}>Payment Details</h3>
              <button onClick={() => setSelected(null)} style={{ background: "none", border: "none", cursor: "pointer", color: "#888" }}><X size={20} /></button>
            </div>

            {[
              ["Transaction ID", <span style={{ fontFamily: "monospace", fontSize: 12, wordBreak: "break-all" }}>{selected.transactionId}</span>],
              ["Student ID",   selected.studentId || "—"],
              ["Full Name",      selected.name],
              ["Email",          selected.email],
              ["Programme",      selected.programme],
              ["Amount",         `GH₵ ${fmt(Number(selected.amount) || 0)}`],
              ["Payment Method", selected.method === "card" ? "Debit / Credit Card" : "Mobile Money"],
              ...
              <div key={label} style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderBottom: "1px solid #f0f0f0", fontSize: 14 }}>
                <span style={{ color: "#666", flexShrink: 0, marginRight: 16 }}>{label}</span>
                <span style={{ fontWeight: 500, textAlign: "right" }}>{val}</span>
              </div>
            ))}

            <div style={{ marginTop: 20 }}>
              <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 10 }}>Update Status</div>
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => { updateStatus(selected.id, "Confirmed"); setSelected(p => ({ ...p, status: "Confirmed" })); }}
                  style={{ flex: 1, background: selected.status === "Confirmed" ? "#e8f5ee" : "#f5f5f5", color: selected.status === "Confirmed" ? "#1B6B3A" : "#555", border: `1.5px solid ${selected.status === "Confirmed" ? "#a8d5b8" : "#ddd"}`, borderRadius: 8, padding: "9px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 5 }}><Check size={14} /> Confirmed</span>
                </button>
                <button onClick={() => { updateStatus(selected.id, "Pending"); setSelected(p => ({ ...p, status: "Pending" })); }}
                  style={{ flex: 1, background: selected.status === "Pending" ? "#fdf3e0" : "#f5f5f5", color: selected.status === "Pending" ? "#b5700a" : "#555", border: `1.5px solid ${selected.status === "Pending" ? "#e8d5a0" : "#ddd"}`, borderRadius: 8, padding: "9px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 5 }}><AlertTriangle size={14} /> Pending</span>
                </button>
                <button onClick={() => { removePayment(selected.id); setSelected(null); }}
                  style={{ background: "#fdecea", color: "#c0392b", border: "1px solid #f5b7b1", borderRadius: 8, padding: "9px 14px", fontSize: 13, cursor: "pointer" }}>
                  <Trash2 size={14} />
                </button>
              </div>
            </div>
