    ...
    <main>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-ug-navy to-ug-blue text-white py-[72px] pb-14">
        ...
        <div
          ...
          style={{ backgroundImage: "url('https://images.unsplash.com/photo-1644174547761-de211415598e?auto=format&fit=crop&w=1600&q=80')" }}
        />
      ...
      <div className="container section">
        ...
        <div className="mb-[52px]">
          <div className="mb-5">
            <span className="badge badge-blue inline-block mb-2">Parallel Tracks</span>
            <h3 className="m-0 text-[1.2rem] text-ug-blue">Presentation Tracks</h3>
          </div>
          <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))" }}>
            {[
              ...
              >
                <span className="flex-shrink-0 mt-0.5 flex items-center" style={{ color: t.color }}>{t.icon}</span>
                <div>
                  <div className="font-bold text-[14px] mb-[5px]" style={{ color: t.color }}>{t.name}</div>
                  <div className="text-[13px] text-[#555] leading-[1.55]">{t.desc}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="flex justify-between items-center flex-wrap gap-3 mb-8">
          <div className="flex gap-2 flex-wrap items-center">
            ...
            {filterOptions.map(f => (
              <button key={f} onClick={() => setFilter(f)} className="border-[1.5px] border-[#ddd] rounded-3xl px-4 py-1.5 text-[13px] font-medium cursor-pointer transition-all duration-150"
                style={{
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => window.print()} className="inline-flex items-center gap-1.5 bg-white text-[#555] border-[1.5px] border-[#ddd] rounded-lg px-[14px] py-[7px] text-[13px] font-semibold cursor-pointer transition-[background] duration-150">
        ...
        {days.map((d, di) => (
          <div key={di} className="mb-12">
            <div className="bg-ug-navy text-white px-6 py-4 rounded-t-xl flex items-center gap-4">
              <span className="bg-ug-gold text-white rounded-lg px-[14px] py-1 font-bold text-[14px]">{d.day}</span>
            ...
            <div className="border border-[#e0e0e0] border-t-0 rounded-b-xl overflow-hidden">
              {d.sessions.filter(matchesFilter).map((s, si) => {
                ...
                return (
                  <div key={si} className="session-row grid items-center px-5 py-[14px] gap-4"
                    style={{
                      gridTemplateColumns: "100px 1fr auto",
                      background: si % 2 === 0 ? "#fff" : "#fafafa",
                      borderBottom: si < d.sessions.length - 1 ? "1px solid #f0f0f0" : "none",
                    }}>
                    <div className="text-[13px] font-medium text-[#555] flex-shrink-0">{s.time}</div>
                    <div>
                      <div className="text-[14px] font-semibold text-[#1a1a1a]">{s.title}</div>
                      {s.presenter && <div className="text-[12px] text-[#888] mt-0.5">{s.presenter}</div>}
                      {s.track && (
                        <span className="inline-block mt-1 text-[11px] font-medium px-[10px] py-[2px] rounded-[20px]"
                          style={{
                    ...
                    <span className="text-[11px] font-semibold px-[10px] py-[3px] rounded-[20px] whitespace-nowrap"
