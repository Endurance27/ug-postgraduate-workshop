    ...
    <main>
      ...
      <div className="container section">
        ...
        <div className="mb-16">
          ...
          <p className="text-[var(--text-muted)] text-sm mb-9">Primary supporters of the 2nd Annual DCS Postgraduate Workshop</p>

          <div className="grid gap-6" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))" }}>
            {(sponsors.length > 0 ? sponsors : [
              { id: 1, name: "University of Ghana",                        role: "Host Institution",      desc: "The University of Ghana, founded in 1948, is the premier research university in Ghana and one of the leading universities in Africa.", tier: "gold",    logo: <Landmark size={40} className="text-ug-gold" /> },
              { id: 2, name: "School of Physical & Mathematical Sciences", role: "Faculty Sponsor",        desc: "SPMS provides direct academic and logistical support for the workshop through its faculty committee.",                               tier: "gold",    logo: <FlaskConical size={40} className="text-ug-gold" /> },
              { id: 3, name: "Department of Computer Science",             role: "Organising Department",  desc: "The Department of Computer Science at UG is the primary organiser of the workshop.",                                                tier: "primary", logo: <Laptop size={40} className="text-ug-blue" /> },
            ]).map((s, i) => (
              <div
                key={i}
                className="card relative overflow-hidden"
                style={{ borderTop: `4px solid ${s.tier === "gold" ? "#C9A84C" : "#1B3A6B"}` }}
              >
                ...
                <h3 className="font-serif text-[1.05rem] mb-2.5 text-ug-blue">{s.name}</h3>
                <p className="text-[13px] text-[var(--text-muted)] leading-[1.75] m-0">{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-ug-navy to-ug-blue rounded-[18px] p-[40px_44px] mb-16 flex items-center gap-9 flex-wrap">
          <div className="flex-shrink-0 text-ug-gold"><BookOpen size={52} /></div>
          <div className="flex-1 min-w-[240px]">
            ...
            <p className="text-white/70 text-sm leading-[1.75] m-0">
              Accepted papers from the workshop are considered for publication in the <strong className="text-ug-gold">College of Basic and Applied Sciences (CBAS) Journal</strong>.
        ...
        <div className="mb-16">
          ...
          <p className="text-[var(--text-muted)] text-sm mb-9">Corporate and industry partners supporting postgraduate research in Ghana</p>

          <div className="bg-[#f9f9fb] border-2 border-dashed border-[#d0d8e8] rounded-2xl p-[48px_36px] text-center">
            <div className="mb-4 flex justify-center text-ug-blue"><Handshake size={48} /></div>
        ...
        <div className="mb-16">
          ...
          <p className="text-[var(--text-muted)] text-sm mb-9">Support the next generation of Ghanaian computer scientists</p>
          ...
          <div className="grid gap-5" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))" }}>
            {[
              ...
              <div key={i} className="rounded-[14px] overflow-hidden shadow-[0_2px_16px_rgba(0,0,0,0.08)] border border-[#eee]">
                <div className="p-[18px_20px] flex items-center gap-2.5" style={{ background: p.color }}>
                  <span className="text-white flex items-center">{p.icon}</span>
                  <span className="font-serif font-bold text-base text-white">{p.tier}</span>
                </div>
                <div className="p-[18px_20px] bg-white">
                  {p.perks.map((perk, pi) => (
                    <div key={pi} className="flex gap-2 mb-2.5 items-start">
                      <span className="flex-shrink-0 mt-px flex items-center" style={{ color: p.color }}><Check size={13} /></span>
                      <span className="text-[13px] text-[var(--text)] leading-[1.5]">{perk}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="text-center bg-ug-blue-light rounded-2xl p-[40px_32px]">
          ...
          <div className="flex gap-3.5 justify-center flex-wrap">
            ...
            {navigate && (
              <button onClick={() => navigate("contact")} className="btn-outline inline-flex items-center gap-1.5">
